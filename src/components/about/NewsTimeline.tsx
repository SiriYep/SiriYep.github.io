...
const formatDate = (dateString: string = "", language = "en"): string => {
  ...
  if (parts.length === 2 && months[parts[0]]) {
    return formatter.format(new Date(`${parts[1]}-${months[parts[0]]}-01T00:00:00Z`));
  }

  const isoMonth = dateString.match(/^(\d{4})-(\d{2})(?:-\d{2})?$/);
  if (isoMonth) {
    return formatter.format(new Date(`${isoMonth[1]}-${isoMonth[2]}-01T00:00:00Z`));
  }
...
const NewsTimeline: React.FC<NewsTimelineProps> = ({ news, showHeader: _showHeader = false }) => {
  ...
  const getNewsTag = useCallback((item: NewsItem) => {
    const type = item.type.toLowerCase();
    const count = item.count ?? 1;
    const fallbackLabel = type === 'publication' ? 'Paper' : type === 'experience' ? 'Intern' : 'Update';
    const label = (item.badge || fallbackLabel)
      .replace(/[\u{1F300}-\u{1FAFF}\u{2600}-\u{27BF}\u{FE00}-\u{FE0F}\u{1F900}-\u{1F9FF}\u{200D}\u{20E3}]/gu, '')
      .trim() || fallbackLabel;
    if (type === 'experience') return label;
    if (isNarrowScreen) return String(count);
    return `${count} ${label}`;
  }, [isNarrowScreen]);
  ...
  const getNewsIcon = (item: NewsItem) => {
    ...
    return item.icon || typeColors[type]?.icon || typeColors.default.icon;
  };
