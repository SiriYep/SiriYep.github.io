              ...
              {pub.authors.map((author: string, idx: number) => {
                ...
                return (
                  <Text
                    as="span"
                    key={idx}
                    fontWeight={isSelfAuthor ? 'bold' : isCoFirstAuthor ? 'semibold' : 'normal'}
                    color={isSelfAuthor ? selfAuthorColor : isCoFirstAuthor ? coFirstColor : undefined}
                  >
                    {cleanAuthor}{(isCoFirstAuthor || hasAsterisk) && <Text as="sup" fontSize="2xs" color="cyan.400">*</Text>}{idx < pub.authors.length - 1 && ', '}
                  </Text>
                )
              })}
            </Text>
